...
ScheduleResult run_preemptive_priority(Process *processes, int count) {
    ...
    while (done < count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (result.processes[idx].arrival_time <= time && remaining[idx] > 0) {
                ...
                if (result.processes[idx].priority < result.processes[chosen].priority) {
                    chosen = idx;
                } else if (result.processes[idx].priority == result.processes[chosen].priority) {
                    if (result.processes[idx].arrival_time < result.processes[chosen].arrival_time) {
                        chosen = idx;
